...
app = FastAPI(title="Specter's Bridge AI Lab")

# Mount static folder (will contain index.html, style.css, app.js)
...
# Shared global environments and evolver state (initialized dynamically)
...
class StepRequest(BaseModel):
    agent_type: str # "player", "expectiminimax", "qlearning", "heuristic", "random"
    intended_move: Optional[int] = None # Used for "player"
    state: Tuple[int, bool, int, int]
    weights: Optional[Tuple[float, float, float, float]] = None # Optional weights for expectiminimax/heuristic
...
@app.get("/")
def read_root():
    """Serves the main landing page of the dashboard."""
...
def reset_game(config: BoardSetupRequest):
    ...
    # Reset Q-agent to sync with new environment
    global_q_agent = QLearningAgent(global_env)
    global_ga_evolver = None
...
def play_step(req: StepRequest):
    ...
    # 1. Determine intended move based on agent type
    intended_move = None
    if req.agent_type == "player":
        ...
        # Make immediate greedy move looking 1-step ahead using heuristic values
    ...
    # 2. Resolve chance outcome stochastically
...
def train_qlearning(req: TrainRequest):
    ...
    # Temporarily instantiate environment to train on the requested config
    train_env = BridgeEnv(
...
if __name__ == "__main__":
    import uvicorn
    # Set host and port for local developer server runs
